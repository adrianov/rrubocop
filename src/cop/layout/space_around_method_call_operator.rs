...
fn op_span(bytes: &[u8], recv: Node<'_>, method: Node<'_>) -> Option<(usize, usize)> {
    ...
    let op_start = if dot > 0 && bytes[dot - 1] == b'&' { dot - 1 } else { dot };
    Some((op_start, dot + 1))
}

fn strip_spaces(
    corr: &mut Vec<Correction>, cop_name: &'static str,
    recv_end: usize, op_start: usize, op_end: usize, method_start: usize,
    before: bool, after: bool,
) {
    if before {
        corr.push(Correction {
            start: recv_end, end: op_start, replacement: String::new(),
            cop_name, cop_index: 0,
        });
    }
    if after {
        corr.push(Correction {
            start: op_end, end: method_start, replacement: String::new(),
            cop_name, cop_index: 0,
        });
    }
}
...
impl Cop for SpaceAroundMethodCallOperator {
    ...
    fn check_node(
        ...
        let Some((op_start, op_end)) = op_span(bytes, recv, method) else { return; };
        ...
        if let Some(corr) = corrections.as_mut() {
            strip_spaces(corr, self.name(), recv.end_byte(), op_start, op_end, method.start_byte(), before, after);
            diag.corrected = true;
        }
