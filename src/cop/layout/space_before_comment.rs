...
fn needs_space(bytes: &[u8], start: usize) -> bool {
    ...
    if prev == b'\n' || prev == b'\r' { return false; }
    if line_prefix(bytes, start).iter().all(|&b| b == b' ' || b == b'\t') { return false; }
    let is_char_esc = start >= 3 && bytes[start - 2] == b'\\' && bytes[start - 3] == b'?';
    (prev != b' ' && prev != b'\t') || (prev == b' ' && is_char_esc)
}

fn check_one(
    cop: &dyn Cop, source: &SourceFile, bytes: &[u8], start: usize,
    diagnostics: &mut Vec<Diagnostic>, corrections: &mut Option<&mut Vec<Correction>>,
) {
    if !needs_space(bytes, start) { return; }
    report::insert_space(
        cop, source, start,
        "Put a space before an end-of-line comment.".into(),
        diagnostics, corrections, start,
    );
}

impl Cop for SpaceBeforeComment {
    fn name(&self) -> &'static str { "Layout/SpaceBeforeComment" }
    ...
    fn check_source(
        ...
        for comment in shared::collect_comments(tree.root_node()) {
            check_one(self, source, bytes, comment.start_byte(), diagnostics, &mut corrections);
        }
    }
}
