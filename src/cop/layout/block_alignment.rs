...
fn end_aligned(style: &str, end_col: usize, block_col: usize, line_col: usize) -> bool {
    match style {
        "start_of_block" => end_col == block_col,
        "start_of_line" => end_col == line_col,
        _ => end_col == block_col || end_col == line_col,
    }
}
...
impl Cop for BlockAlignment {
    ...
    fn check_node(
        ...
        let block_col = shared::node_col(source, node);
        if end_aligned(
        ...
        report::fix_indent(
            ...
            source,
            end_kw.start_byte(),
            format!("`end` is not aligned with `do` beginning at column {block_col}."),
            diagnostics,
            &mut corrections,
